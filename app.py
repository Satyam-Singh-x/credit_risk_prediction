...
<div style="background: linear-gradient(135deg, #1a1a2e, #16213e, #0f3460);
            ...
            border: 1px solid #e74c3c; margin-bottom: 1rem;">
...
@st.cache_resource
def load_models():
    # ── Relative paths — works on laptop AND Streamlit Cloud ───────
    ...
    shap_importance = pd.read_csv(
        os.path.join(shap_out, 'shap_importance.csv')
    )

    explainer = shap.TreeExplainer(lgb_model)

    return [lgb_model], feature_names, best_threshold, \
            explainer, shap_importance
...
def engineer_features(inputs):
    ...
    Takes raw user inputs → returns full feature vector.
    """
    d = inputs.copy()
    ...
    # ── Build full feature vector aligned to training ──────────────
...
with col1:
    ...
    AMT_ANNUITY       = st.number_input(
        "Monthly Annuity (₹)", min_value=1000,
        max_value=200000, value=25000, step=1000)
    AMT_GOODS_PRICE   = st.number_input(
        "Goods Price (₹)", min_value=10000,
        max_value=4000000, value=450000, step=10000)
...
with col2:
    ...
    DAYS_REGISTRATION     = st.slider(
        "Years Since Registration", 0, 30, 10) * -365
    DAYS_ID_PUBLISH       = st.slider(
        "Years Since ID Published", 0, 20, 5) * -365
    DAYS_LAST_PHONE_CHANGE = st.slider(
        "Years Since Phone Change", 0, 10, 2) * -365
...
with col3:
    ...
    EXT_SOURCE_1 = st.slider(
        ...
        0.0, 1.0, 0.5, 0.01,
        help="Third-party credit score (0=worst, 1=best)")
    EXT_SOURCE_2 = st.slider(
        "External Credit Score 2 ⭐",
        0.0, 1.0, 0.5, 0.01,
        help="Most important predictor!")
...
if predict_clicked:
    ...
    with st.spinner("Computing default probability..."):
        ...
        risk_label, risk_class, risk_emoji = get_risk_category(
            prob, best_threshold
        )
    ...
    # ── Probability gauge ──────────────────────────────────────────
    st.markdown('<div class="section-title">📊 Risk Probability Gauge</div>',
                unsafe_allow_html=True)
    ...
    zones = [(0.00, 0.20, '#d5f5e3', 'Very Safe'),
    ...
    for x_start, x_end, color, label in zones:
        ax.barh(0.5, x_end - x_start, left=x_start,
                height=0.6, color=color, edgecolor='white', linewidth=2)
    ...
    ax.text(prob, 0.1, f'{prob:.1%}',
            ha='center', va='bottom',
            fontsize=12, fontweight='bold', color='#2c3e50')
    ax.axvline(best_threshold, color='#e74c3c',
               linewidth=2, linestyle='--', ymin=0.1, ymax=0.9,
    ...
    # ══════════════════════════════════════════════════════════════
    st.markdown('<div class="section-title">💡 Key Risk Indicators</div>',
                unsafe_allow_html=True)
    ...
    r1.metric("Credit / Income Ratio",
              f"{credit_income:.2f}x",
              "⚠️ High"   if credit_income  > 3  else "✅ Normal",
              delta_color="off")
    r2.metric("Monthly Burden",
              f"{annuity_income:.1f}% of income",
              "⚠️ High"   if annuity_income > 30 else "✅ Normal",
              delta_color="off")
    r3.metric("Loan Term",
              f"{credit_term:.0f} months",
              "⚠️ Long"   if credit_term   > 48 else "✅ Short",
              delta_color="off")
    r4.metric("Avg Credit Score",
              f"{ext_mean:.2f}",
              "⚠️ Low"    if ext_mean       < 0.4 else "✅ Good",
              delta_color="off")
    ...
    ax.set_xlabel('SHAP Value  '
                  ...
                  fontsize=10)
    ax.set_title(f'Top 15 Features Driving This Prediction\n'
                 f'Customer Default Probability: {prob:.1%}',
    ...
    with exp_col1:
        ...
        for _, row in top_risk_features.iterrows():
            # ── FIX: safely get feature value ─────────────────────
            if row['feature'] in input_df.columns:
                feat_val = float(input_df[row['feature']].values[0])
            ...
            st.markdown(
                ...
                f'⬆️ <b>{row["feature"]}</b><br>'
                f'Value &nbsp;: <b>{val_str}</b><br>'
                f'SHAP &nbsp;&nbsp;: '
                f'<b style="color:#e74c3c">+{row["shap"]:.4f}</b> '
                f'— pushes toward default'
                f'</div>',
                unsafe_allow_html=True
            )
    ...
    with exp_col2:
        ...
        for _, row in top_protect_features.iterrows():
            ...
            st.markdown(
                ...
                f'Value &nbsp;: <b>{val_str}</b><br>'
                f'SHAP &nbsp;&nbsp;: '
                f'<b style="color:#2ecc71">{row["shap"]:.4f}</b> '
                f'— pulls away from default'
                f'</div>',
                unsafe_allow_html=True
            )
